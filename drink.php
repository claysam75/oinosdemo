...
if (!isset($_SESSION['loggedin'])) {
    header('Location: index.php?redirect=drink.php');
    exit;
}
...
if($_POST['submitDrink']) {
    $adjustValue = $_POST['existing-wine-adjustValue'];
    $ean1 = $_POST['existing-wine-ean'];
    $stmt=$conn->prepare("SELECT stock FROM wine WHERE ean = ?");
    $stmt->bind_param('s', $ean1);
    $stmt->execute();
    $result = $stmt->get_result();
    $stockResult = $result->fetch_object();
    $stockValue = $stockResult->stock;
    $stmt->close();
    $newStock = $stockValue - $adjustValue;
    
    $stmt=$conn->prepare("UPDATE wine SET stock = ? WHERE ean = ?");
    $stmt->bind_param('ss', $newStock, $ean1);
    $stmt->execute();
    $stmt->close();
...
if ($thisean) {
    ...
    if($numRows > 0) {
        ...
        $stmt->close();
        $queryOutput = '
        <div class="card m-2 drinkWineTrue" id="queryOutputTrue">
        <div class="card-body">
            <h4 class="card-title">
                Drink 
            </h4>
            ...
            <form action="drink.php" method="post">
                ...
                <div class="form-group">
                <label for="existing-wine-adjustValue">Reduce by</label>
                <div class="input-group">
                    
                    <div class="input-group-prepend" id="existing-wine-valueAdjust">
                        <button class="btn btn-danger" type="button" id="valueDecrease" onclick="valDecrement()">-</button>
                    </div>
                    <input type="tel" class="form-control" name="existing-wine-adjustValue" id="existing-wine-adjustValue" style="text-align:center;" min="1" max="999" value="1">
                    <div class="input-group-append">
                        <button class="btn btn-success" type="button" id="valueIncrease" onclick="valIncrement()">+</button>
                    </div>
                </div>
                </div>
                <br>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <button type="submit" name="submitDrink" id="submitDrink" value="Save" class="btn btn-block btn-primary form-group"><i class="fas fa-save"></i> Save</button>
                    </div>
...
<html lang=en>
    ...
    <body style="margin-bottom: 70px">
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            <a class="navbar-brand" href="drink.php">
            ...
            $(function() {
                ...
                function validateEan(barcode) {
                    ...
                    if (checkSum == lastDigit) {
                        ...
                        window.location.href="drink.php?ean=" + ean;
                    } else {
                        console.log("Couldn't understand barcode");
                        window.location.href="drink.php?message=failcode"
        ...
        function valDecrement() {
                var currentVal1 = parseInt(document.getElementById("existing-wine-adjustValue").value);
                console.log(currentVal1);
                var newVal = currentVal1 -= 1;
                if(newVal <= 0) {
                    document.getElementById("existing-wine-adjustValue").value = 1;
                } else {
                document.getElementById("existing-wine-adjustValue").value = newVal;
        };
            };

            function valIncrement() {
                var currentVal2 = parseInt(document.getElementById("existing-wine-adjustValue").value);
                console.log(currentVal2);
                var newVal = currentVal2 += 1;
                document.getElementById("existing-wine-adjustValue").value = newVal;
            };
        </script>
